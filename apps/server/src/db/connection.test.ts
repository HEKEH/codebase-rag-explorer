...
describe("db/connection", () => {
  test("creates database file and initializes required tables", () => {
    ...
    const migrationRows = db
      .query<{ id: string }, []>("SELECT id FROM schema_migrations ORDER BY id ASC")
      .all()
      .map((row) => row.id);
    db.close();

    expect(rows.map((row) => row.name).sort()).toEqual(["chunks", "embeddings", "repos", "schema_migrations"]);
    expect(repoColumns.includes("updated_at")).toBe(true);
    expect(migrationRows).toEqual(["001_initial_schema.sql", "002_add_repos_updated_at.sql"]);
