...
async function loadModules(dbPath: string) {
  ...
  const indexModule = await import(pathToFileURL(join(testCwd, "apps/server/src/index.ts")).href + cacheBuster);
  const repoModule = await import(pathToFileURL(join(testCwd, "apps/server/src/db/repo.repository.ts")).href + cacheBuster);
  const askModule = await import(pathToFileURL(join(testCwd, "apps/server/src/services/ask.service.ts")).href + cacheBuster);
  const indexServiceModule = await import(pathToFileURL(join(testCwd, "apps/server/src/services/index.service.ts")).href + cacheBuster);
  const connectionModule = await import(pathToFileURL(join(testCwd, "apps/server/src/db/connection.ts")).href + cacheBuster);
  const errorsModule = await import(pathToFileURL(join(testCwd, "apps/server/src/lib/errors.ts")).href + cacheBuster);
  const enumsModule = await import(pathToFileURL(join(testCwd, "packages/types/src/enums.ts")).href + cacheBuster);
  ...
  return {
    ...
    AskService: askModule.AskService as { prototype: { ask: (...args: unknown[]) => Promise<unknown> } },
    IndexService: indexServiceModule.IndexService as { prototype: { buildIndex: (...args: unknown[]) => Promise<unknown> } },
    closeDb: connectionModule.closeDb as () => void,
    AppError: errorsModule.AppError as new (code: number, message: string) => Error,
    ErrorCode: enumsModule.ErrorCode as Record<string, number>
  };
}

function assertFailurePayload(payload: { code: number; message: unknown; data: unknown }, expectedCode: number) {
...
describe("failure protocol consistency", () => {
  test("covers failure codes with {code,message,data:null} and preserves NO_RELEVANT_CODE business payload", async () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "server-protocol-consistency-"));
    ...
    try {
      ...
      const notFoundRes = await app.handle(
        new Request("http://localhost/api/repo/import", {
          ...
          body: JSON.stringify({ path: join(tempRoot, "missing"), type: "local" })
        })
      );
      assertFailurePayload(await notFoundRes.json(), ErrorCode.REPO_LOAD_FAILED);
      ...
      assertFailurePayload(await duplicateImport.json(), ErrorCode.REPO_ALREADY_EXISTS);
      ...
      assertFailurePayload(await indexAgain.json(), ErrorCode.INDEX_ALREADY_EXISTS);
      ...
      assertFailurePayload(await askEmbeddingFailed.json(), ErrorCode.EMBEDDING_FAILED);
      ...
      assertFailurePayload(await askInternalError.json(), ErrorCode.INTERNAL_ERROR);
