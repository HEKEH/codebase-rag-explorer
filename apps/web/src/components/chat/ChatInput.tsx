...
export function ChatInput({ question, canAsk, isLoading, onQuestionChange, onSubmit }: ChatInputProps) {
  return (
    <form data-testid="chat-input-form" onSubmit={onSubmit} className="flex gap-3 w-full">
      <Input
        value={question}
      ...
      />
      <Button type="submit" disabled={isLoading || !canAsk || !question.trim()} className="h-10 px-5">
        {isLoading ? (
