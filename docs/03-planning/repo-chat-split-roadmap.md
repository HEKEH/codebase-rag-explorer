...
- [x] **P3-1** | PRD §11.2 / TRD §4.5 | 引入路由并新增页面：`/repos`、`/chat` | 验收：可导航切换且刷新不丢路由
- [x] **P3-2** | PRD §11.3 | 仓库管理页实现列表、添加、删除、重载 | 验收：四类操作均可触发并反馈状态
- [x] **P3-3** | PRD §11.4.1 | 聊天页实现仓库选择器（展示全部，仅 `indexed` 可选可问答） | 验收：`loaded/indexing/failed` 均不可发问
- [x] **P3-4** | PRD §11.4.1 | 默认仓库恢复：`localStorage.lastOpenedRepoId` | 验收：刷新后恢复上次仓库
- [x] **P3-5** | PRD §11.4.3 | 对话按仓库隔离（`messagesByRepoAtom`） | 验收：切换仓库可恢复各自历史
- [x] **P3-6** | PRD §11.4.3 | 增加“清空当前仓库聊天历史”交互 | 验收：清空后仅当前仓库历史被移除
- [x] **P3-7** | PRD §11.3.2 | 重复添加交互：收到 `1002` 后弹窗询问是否重载 | 验收：确认后调用 reload，取消则不触发
...
- [x] **P4-1** | TRD §7.1 | 前端统一错误映射：`1002/1003/1004/2001/3001` | 验收：各错误有明确中文提示与下一步操作
- [x] **P4-2** | PRD §11.6 | 删除不存在仓库、重载冲突、未索引提问等边界提示优化 | 验收：无“静默失败”
- [x] **P4-3** | docs/06-operations | 记录关键事件日志：create/delete/reload/clear-history/ask-failed | 验收：日志字段可关联 `repo_id`
- [x] **P4-4** | TRD §3.1.7 | 状态轮询与按钮禁用联动 | 验收：indexing 阶段防重复操作
...
> 目标：补齐上线前后关键动作，确保增量改造具备可灰度、可回滚、可观测能力

- [ ] **P6-1** | docs/06-operations | 发布前检查清单（接口可用、错误码映射、前端状态禁用策略） | 验收：清单执行记录可追溯
- [ ] **P6-2** | docs/06-operations | 发布后监控检查（create/delete/reload/ask-failed 关键事件） | 验收：上线后日志可关联 `repo_id` 且无异常峰值
- [ ] **P6-3** | docs/06-operations | 回滚触发条件与操作步骤（含前后端版本回退） | 验收：形成可执行 Runbook

**Phase 6 完成标志**：具备可执行的发布检查、监控核验与回滚预案。
...
- 2026-04-30：完成 P3-1，引入 `react-router-dom` 并落地 `/repos`、`/chat` 双页面路由；新增路由测试覆盖导航切换与刷新后路由保持。
- 2026-04-30：完成 P3-2，仓库管理页支持仓库列表查询与添加/删除/重载操作，并补齐页面交互测试验证四类操作可触发且有状态反馈。
- 2026-04-30：完成 P3-3，聊天页新增仓库下拉选择器并展示全部仓库；最终口径为仅 `indexed` 可选可提问，`loaded/indexing/failed` 禁用不可提问。
- 2026-04-30：完成 P3-4，接入 `localStorage.lastOpenedRepoId` 恢复默认仓库，刷新后可自动回到上次选择。
- 2026-04-30：完成 P3-5，新增 `messagesByRepoAtom` 按仓库存储会话，切换仓库后可恢复各自聊天历史。
- 2026-04-30：完成 P3-6，聊天页新增“清空当前仓库聊天历史”按钮并调用清空接口，仅影响当前仓库会话。
- 2026-04-30：完成 P3-7，仓库重复添加命中 `1002` 时弹窗询问是否重载，确认触发 reload，取消不触发。
- 2026-04-30：新增 `@repo/shared` 共享包并统一前后端 `source_value` 规范化逻辑；修复本地根路径 `/` 归一化为空字符串的问题，并补齐 local/git 规范化回归测试。
- 2026-04-30：完成 P4-1，前端新增统一错误映射 `1002/1003/1004/2001/3001`，确保页面提示包含可执行下一步操作。
...
- 2026-04-30：修正聊天页可问答状态定义：仅 `indexed` 可选可问答，`loaded/indexing/failed` 统一禁用；同步补齐测试与验收文档状态矩阵描述。
- 2026-04-30：补充 Phase 6（发布与回滚检查），覆盖发布前检查、发布后监控核验与回滚 Runbook 约束。
