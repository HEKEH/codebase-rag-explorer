...
- [ ] **P1-2** | IndexService / 写入路径 | 新建或更新 chunk 时 **同步写入/更新稀疏索引**（FTS 行或倒排增量；可检索文本与 `chunk_id` 对齐） | 验收：索引后针对已知词可检索命中（FTS 为 `MATCH`；倒排为等价查询）
- [ ] **P1-3** | 级联删除 / 重载 | 仓库删除、chunk 批量清理、**全量重载重建** 时稀疏索引与 `chunks` **一致** | 验收：删除/重载后无「幽灵」命中；重载完成后索引条目数与 chunk 数一致（或等价无孤儿）
