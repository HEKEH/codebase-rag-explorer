...
export const repoApi = {
  ...
  remove: (repoId: string) =>
    apiClient.request<DeleteRepoData>(
  ...
  reload: (repoId: string) =>
    apiClient.request<BuildIndexData>(
      `/api/repos/${encodeURIComponent(repoId)}/reload`,
      {
        method: "POST",
      },
    ),
  status: (repoId: string) =>
    apiClient.request<IndexStatusData>(
      `/api/repos/${encodeURIComponent(repoId)}/status`,
    ),
  ...
  legacyStatus: (repoId: string) =>
    apiClient.request<IndexStatusData>(
      `/api/index/status?repo_id=${encodeURIComponent(repoId)}`,
    ),
};
